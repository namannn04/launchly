...
type RepoCardProps = {
  ...
  deploymentStatus?: "queued" | "building" | "success" | "failed";
  deploymentUrl?: string | null;
  deploymentError?: string | null;
};

export default function RepoCard({ repo, onImport, deploymentStatus, deploymentUrl, deploymentError }: RepoCardProps) {
  const isDeploying = deploymentStatus === "queued" || deploymentStatus === "building";
      ...
      <Card className="rounded-xl border-border/70 bg-card/85 px-4 py-3 shadow-none">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <Button size="sm" onClick={() => onImport(repo)} disabled={isDeploying}>
              {importButtonLabel}
            </Button>
          </div>
        </div>
        ...
        {deploymentStatus ? (
          ...
          >
            {deploymentStatus === "queued" ? "Deployment queued" : null}
            {deploymentStatus === "building" ? "Deployment in progress" : null}
            {deploymentStatus === "success" ? `Deployed${deploymentUrl ? `: ${deploymentUrl}` : ""}` : null}
            {deploymentStatus === "failed" ? `Deployment failed${deploymentError ? `: ${deploymentError}` : ""}` : null}
          </p>
        ) : null}
      </Card>
    </motion.div>
